...
use crate::config_ressources::{preparer_index_mongodb_hebergement, preparer_queues};
...
pub async fn run() {
    ...
    futures.push(tokio::spawn(thread_entretien(gestionnaire, middleware)));
...
async fn initialiser<M>(middleware: &'static M) -> Result<(&'static GestionnaireDomaineHebergement, FuturesUnordered<JoinHandle<()>>), Error>
    where M: Middleware
{
    let gestionnaire = GestionnaireDomaineHebergement {};
    ...
    preparer_index_mongodb_hebergement(middleware, gestionnaire).await
...
#[derive(Clone)]
pub struct GestionnaireDomaineHebergement {}

#[async_trait]
impl AiguillageTransactions for GestionnaireDomaineHebergement {
...
#[async_trait]
impl GestionnaireDomaineSimple for GestionnaireDomaineHebergement {}

impl GestionnaireDomaineV2 for GestionnaireDomaineHebergement {
    fn get_collection_transactions(&self) -> Option<String> {
        Some(Constantes::COLLECTION_NOM.to_string())
    }
...
impl GestionnaireBusMillegrilles for GestionnaireDomaineHebergement {
    fn get_nom_domaine(&self) -> String {
        Constantes::DOMAINE_NOM.to_string()
    }
...
#[async_trait]
impl ConsommateurMessagesBus for GestionnaireDomaineHebergement {
...
async fn thread_entretien<M>(_gestionnaire: &GestionnaireDomaineHebergement, middleware: &M)
    where M: Middleware
{
